...
向用户展示修正后草稿。用户确认后，使用 `card-generator.py --config config.json -o 角色名.json` 生成 JSON，再用 `--validate` 和 `--list` 验证。
...
使用 `python scripts/world-book-create.py` 执行写入，然后使用 `python scripts/query.py <世界书路径> --brief` 验证配置是否正确。
