...
9. **先读 reference，再动手** — 不凭记忆编写，每个任务类型必须读取其对应的全部 reference 文件后方可执行。任何写入操作之前**必须先读 `references/card-generator-guide.md`**，使用 `card-generator.py`、`query.py`、`world-book-create.py` 来写入角色卡/世界书。
